...
df_te = pd.get_dummies(df_te, columns=['indus']).dropna()
#     代入する
X_test = df_te.drop(['RATE',  'date', 'code', 'index'], axis = 1)


# In[12]:


dffuture2 = pd.concat([df_te,pd.DataFrame(tpot_pre.predict_proba(X_test)[:,1],columns = ['Label'])], axis = 1).dropna()
...
dffuture3 = dffuture2[dffuture2['Label'] > 0.5].sort_values('Label', ascending = False).set_index('index')
...
if len(dffuture3) == 0:
    ...
    for i  in range(10):
        ...
        body_a = body_a + '/Score=' + str(dffuture3.loc[:,'Label'].iloc[i]) + ' :'
