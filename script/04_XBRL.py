...
def process(i):
    ...
    try:
        ...
        code = pd.concat([data_j[data_j['後株'] == name.iloc[0]], data_j[data_j['前株'] == name.iloc[0]]])['コード'].iloc[0]
        date = df['提出日'][0].date()
...
if len(df_new) == 0:
    if len(df_old) == 0:
        ...
        df_old.drop_duplicates().sort_values('code').to_csv('XBRL_list.csv', index = False)
else :
    if len(df_old) == 0:
        df_new.drop_duplicates().sort_values('code').to_csv('XBRL_list.csv', index = False)
    else:
        pd.concat([df_new, df_old]).drop_duplicates().sort_values('code').to_csv('XBRL_list.csv', index = False)
